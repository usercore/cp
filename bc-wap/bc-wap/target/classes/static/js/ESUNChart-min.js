...
JoinLine.prototype = {
	...
	join: function(h, f, g,x) {
		...
		document.getElementById(x).appendChild(this.tmpDom);
	},
	pos: function(c) {
		var d = {
				x: 0,
	...
	FFLine: function(c, g, b, f) {
		...
		var i = Math.min(g, f);
		var a = Math.min(c, b);
		d.style.top = i-$('.trend_heard').height() + "px";
		d.style.left = a + "px";
...
LG.prototype = {
	draw: function(a, b, c) {
		...
		this.line.join(this.g, LG.isShow, c,LG.drawElementId)
	},
	clear: function() {
		this.line.remove()
	},
	...
	draw: function(d) {
		...
		if(d) {
			...
			for(var a = 0; a < c; a++) {
				var b = this.vg[a];
				LG.color = b.color;
				LG.drawElementId = this.drawElementId;
